      ...
      <div className="relative">
        ...
        <div
          className={`z-20 mt-[68px] duration-300 absolute top-0 md:hidden  ${
            isShowHamberMenu ? "w-48 ltr:left-0 rtl:right-0" : "w-0 ltr:-left-52 rtl:-right-52"
          }`}
        >
          <div className="fixed ">
          <SideBar
        ...
        <div onClick={()=>setIsShowHamberMenu((last:any)=>!last)} className={`md:hidden z-10  duration-300 absolute top-0 h-full bg-a_general-30/50 dark:bg-a_general-30/20 ${isShowHamberMenu ? 'w-full' : 'w-0'}`}></div>
      </div>
    </>
  );
};
